...
describe('功能模块发布边界', () => {
  ...
  test('部署入口支持准备和单独发布 DeepSeek Harness (dsh)', () => {
    const prepareDshScript = readFileSync(join(repoRoot, 'scripts/prepare-dsh-module.ts'), 'utf8')
    expect(rootPackage.scripts?.['prepare:dsh-module']).toBeDefined()
    expect(deployShellScript).toContain('--dsh')
    expect(deployShellScript).toContain('prepare:dsh-module')
    expect(deployShellScript).toContain('--dsh-archive')
    expect(deployShellScript).toContain('DeepSeek Harness (dsh) 必须在目标平台和架构准备')
    expect(deployShellScript).toContain('read_dsh_version')
    expect(buildManifestScript).toContain('--dsh')
    expect(deployScript).toContain('[switch]$DshOnly')
    expect(deployScript).toContain("'--dsh'")
    expect(prepareDshScript).toContain("DSH_PACKAGE = '@deepseek-ai/dsh'")
    expect(prepareDshScript).toContain('verifyOfficialPackage')
    expect(prepareDshScript).toContain('COPIS_DSH_NODE')
  })

  test('部署入口支持准备和单独发布专业模式核心组件 (codex-cli)', () => {
    const prepareCodexCliScript = readFileSync(join(repoRoot, 'scripts/prepare-codex-cli-module.ts'), 'utf8')
    expect(rootPackage.scripts?.['prepare:codex-cli-module']).toBeDefined()
    expect(deployShellScript).toContain('--codex-cli')
    ...
    expect(deployScript).toContain("'--codex-cli'")
    expect(prepareCodexCliScript).toContain('resolveSourceCodexBinary')
    expect(prepareCodexCliScript).toContain('CODEX_CLI_VERSION')
  })
})
