...
> 2026-09-10 对照审查：未发现拆分引入的功能回归；381 个 handle/on 调用、展开后的 415 条注册阶段语句与原版一致，34 个辅助函数/状态声明一致；IPC 测试 62 项通过。实际 Electron 窗口交互仍待用户验收。
...
- [x] T1 完成；可独立提交和验证。
- [x] T2 完成；同步 on 和异步 handle 均验证。
- [x] T3 完成；Windows 与 macOS 分支分别通过 mock 验证，真实平台交互待用户确认。
- [x] T4 完成；会话、权限和目录监听回归通过。
- [x] T5 完成；兼容 API、事件与登录清理回归通过。
- [x] T6 完成；输入校验、文件副作用与同步临界区回归通过。
- [x] T7 完成；新旧源码达标并保留全部对外入口。

## 6. 启动副作用迁移细则

不能简单将整个 registerIpcHandlers 内容按注释段落搬走。当前函数除了注册，还执行主题订阅、自动归档、定时器、失效路径清理、临时文件清理和 DSH broadcaster 设置。

- [x] 自动归档与存储清理分别提取为 `startIpcArchiveMaintenance(): void`、`startIpcStorageCleanup(): void`，在两个原位置调用，保留中间的 storage 注册顺序；导入无副作用。
- [x] 用 fake timer 和 mock 服务验证立即归档、24 小时周期、清理开关及异常不中断启动，不在真实用户配置上测试。
- [x] 主题订阅随 settings 域，DSH broadcaster 随 dsh 域；在实际入口只调用一次。
- [x] 保留原应用关闭清理策略。
...
- [x] ipc.ts 与所有新增手写源文件 ≤600 行；入口、注册器、平台逻辑和启动维护各自职责清晰。
- [x] 注册清单无丢失/重复，原导出和 preload 方法兼容；代表性运行测试覆盖参数、返回、错误、事件和取消/同步行为。
...
- [x] 回写总计划中的完成状态时保留历史扫描值和新验证记录。自动化完成与用户 UI 待确认分别标注。

完成状态以执行记录中的实际迁移与验证结果为准；尚未勾选的阶段仍需实施。
