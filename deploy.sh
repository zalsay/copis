...
AGENTLY_CLI_ONLY="${COPIS_AGENTLY_CLI_ONLY:-0}"
DSH_ONLY="${COPIS_DSH_ONLY:-0}"
CODEX_CLI_ONLY="${COPIS_CODEX_CLI_ONLY:-0}"
...
DSH_ARCHIVE="${COPIS_DSH_ARCHIVE:-}"
DSH_VERSION="${COPIS_DSH_VERSION:-}"
CODEX_CLI_ARCHIVE="${COPIS_CODEX_CLI_ARCHIVE:-}"
CODEX_CLI_VERSION="${COPIS_CODEX_CLI_VERSION:-}"
CODEX_CLI_BINARY="${COPIS_CODEX_CLI_BINARY:-}"
...
read_dsh_version() {
  local archive runtime_metadata
  archive="$1"
  runtime_metadata="$(LC_ALL=C tar -xOf "$archive" ./runtime/node_modules/@deepseek-ai/dsh/package.json 2>/dev/null)"
  if [[ -z "$runtime_metadata" ]]; then
    fail "无法读取 DeepSeek Harness (dsh) 归档版本：$archive"
  fi
  "$BUN_BIN" -e '
    const metadata = JSON.parse(process.argv[1])
    if (typeof metadata.version !== "string" || !/^\d+\.\d+\.\d+/.test(metadata.version)) {
      throw new Error("dsh package.json 缺少 version")
    }
    const match = metadata.version.match(/^(\d+\.\d+\.\d+)/)
    console.log(match ? match[1] : metadata.version)
  ' "$runtime_metadata"
}
...
if [[ "$RUST_ONLY" != '1' && "$OFFICECLI_ONLY" != '1' && "$NODE_RUNTIME_ONLY" != '1' && "$ALIPAY_BOT_ONLY" != '1' && "$PLAYWRIGHT_CORE_ONLY" != '1' && "$PYTHON_RUNTIME_ONLY" != '1' && "$AGENTLY_CLI_ONLY" != '1' && "$CODEX_CLI_ONLY" != '1' ]]; then
  ...
  DSH_ARCHIVE="${DSH_ARCHIVE:-$APP_DIR/resources/dsh/${PLATFORM}-${ARCH}.tar.gz}"
  if [[ ! -f "$DSH_ARCHIVE" || "${COPIS_REFRESH_DSH:-0}" == '1' ]]; then
    if [[ "$PLATFORM" != "$CURRENT_PLATFORM" || "$ARCH" != "$CURRENT_ARCH" ]]; then
      fail 'DeepSeek Harness (dsh) 必须在目标平台和架构准备，跨平台请传入 --dsh-archive。'
    fi
    DSH_METADATA="$(mktemp "${TMPDIR:-/tmp}/copis-dsh-metadata.XXXXXX")"
    echo '[Copis] 正在准备官方 DeepSeek Harness (dsh) 功能模块...'
    run_bun "$ROOT_DIR" 'DeepSeek Harness (dsh) 功能模块准备失败' run prepare:dsh-module -- \
      --platform "$PLATFORM" --arch "$ARCH" --output "$DSH_ARCHIVE" --metadata "$DSH_METADATA"
    DSH_VERSION="${DSH_VERSION:-$(cd "$ROOT_DIR" && "$BUN_BIN" -e 'const metadata = JSON.parse(await Bun.file(process.argv[1]).text()); console.log(metadata.version)' "$DSH_METADATA")}"
    rm -f "$DSH_METADATA"
  fi
  if [[ ! -f "$DSH_ARCHIVE" ]]; then
...
if [[ "$SKIP_PUBLISH" -eq 0 ]]; then
  ...
  if [[ "$AGENTLY_CLI_ONLY" == '1' ]]; then
    RELEASE_ARGS+=(--agently-cli)
  fi
  if [[ "$DSH_ONLY" == '1' ]]; then
    RELEASE_ARGS+=(--dsh)
  fi
