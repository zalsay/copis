...
if [[ "$SKIP_PUBLISH" -eq 0 ]]; then
  ...
  run_bun "$ROOT_DIR" '功能模块 manifest 构建失败' run build:functional-module-manifest -- "${RELEASE_ARGS[@]}"
  echo '[Copis] 正在发布功能模块二进制与 manifest 到 COS...'
  run_bun "$ROOT_DIR" '功能模块 COS 发布失败' run publish:functional-modules -- "${RELEASE_ARGS[@]}"
  echo "[Copis] 功能模块发布完成：$PLATFORM/$ARCH $VERSION"
else
