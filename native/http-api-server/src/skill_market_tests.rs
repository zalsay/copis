    use super::{
        ...
        validate_skill_slug, MARKET_SOURCE_FILE, SkillMarketRoute, SkillMarketState,
    };
    use std::fs;
    ...
    ) -> (mpsc::Receiver<()>, Arc<AtomicBool>, std::thread::JoinHandle<()>) {
        let (ready_tx, ready_rx) = mpsc::channel();
    ...
    fn local_install_failure_rolls_back_remote_install() {
        ...
        let suffix = format!("{}-{}", std::process::id(), super::unique_suffix());
        let config_dir =
            std::env::temp_dir().join(format!("copis-skill-market-rollback-{}", suffix));
        let workspace_dir = config_dir.join("agent-workspaces").join("demo");
        fs::create_dir_all(&workspace_dir).unwrap();
        fs::write(
            config_dir.join("agent-workspaces.json"),
        ...
        let (ready_rx, stop, backend) =
            spawn_market_mock_backend(listener, Arc::clone(&delete_calls));
        ready_rx.recv().unwrap();
