...
pub fn worker_automation_context(
    session_id: &str,
    token: &str,
) -> Result<WorkerAutomationContext, AutomationError> {
...
impl AutomationStore {
    ...
    pub fn create_for_session(
        ...
        if triggered_by == "automation" {
            return Err(AutomationError::Validation(
                "当前是定时任务自动执行，禁止递归创建新的定时任务".to_string(),
            ));
        }
        ...
        let active = input.active.unwrap_or(true)
            && runnable(&input.channel_id, input.workspace_id.as_deref());
    ...
    pub fn update(&self, id: &str, input: AutomationUpdateInput) -> Result<Value, AutomationError> {
        ...
        let previous_active = target
            ...
            .unwrap_or(false);
        let schedule_changed = input.schedule_type.as_deref() != None
            && input.schedule_type.as_deref() != value_string(target, "scheduleType")
            || input.interval_minutes.is_some_and(|value| {
                Some(value) != target.get("intervalMinutes").and_then(Value::as_u64)
            })
            || input.time_of_day.as_deref() != None
                && input.time_of_day.as_deref() != value_string(target, "timeOfDay")
            || input.day_of_week.is_some_and(|value| {
                Some(value) != target.get("dayOfWeek").and_then(Value::as_u64)
            })
            || input.day_of_month.is_some_and(|value| {
                Some(value) != target.get("dayOfMonth").and_then(Value::as_u64)
            })
            || input.scheduled_at.is_some_and(|value| {
                Some(value) != target.get("scheduledAt").and_then(Value::as_u64)
            });
        ...
        if !runnable(
            value_string(target, "channelId").unwrap_or(""),
            value_string(target, "workspaceId"),
        ) {
    ...
    pub fn append_run(&self, id: &str, run: AutomationRunInput) -> Result<Value, AutomationError> {
        ...
        if let Some(duration_ms) = run.duration_ms {
            entry.insert("durationMs".to_string(), json!(duration_ms));
        }
        if let Some(error) = run.error {
            entry.insert("error".to_string(), json!(error));
        }
        if let Some(skip_reason) = run.skip_reason {
            entry.insert("skipReason".to_string(), json!(skip_reason));
        }
        history.insert(0, Value::Object(entry));
        history.truncate(MAX_HISTORY);
        set_value(target, "runHistory", Value::Array(history));
        if target.get("runHistory").and_then(Value::as_array).is_some()
            && target.get("runHistory").is_some()
        {
            if run.status != "skipped" {
                let run_count = target
                    .get("runCount")
                    .and_then(Value::as_u64)
                    .unwrap_or(0)
                    .saturating_add(1);
                ...
                let complete = target.get("scheduleType").and_then(Value::as_str) == Some("once")
                    || target
                        .get("maxRuns")
                        .and_then(Value::as_u64)
                        .map(|maximum| run_count >= maximum)
                        .unwrap_or(false);
                if complete {
                    set_bool(target, "active", false);
                    set_number(target, "completedAt", now_millis());
                } else {
                    let schedule_type = value_string(target, "scheduleType")
                        .unwrap_or("interval")
                        .to_string();
                    let interval = target
                        .get("intervalMinutes")
    ...
    fn read_index(&self) -> Result<AutomationIndex, AutomationError> {
        ...
        let value: Value = serde_json::from_str(&raw)
            .map_err(|_| AutomationError::Storage("定时任务索引不是合法 JSON".to_string()))?;
        let version = value
            .get("version")
            .and_then(Value::as_u64)
            .unwrap_or(INDEX_VERSION);
        let mut automations = value
            .get("automations")
            .and_then(Value::as_array)
            .cloned()
            .unwrap_or_default();
        ...
        for automation in &mut automations {
            migrated |= migrate_legacy_fields(automation);
        }
        let index = AutomationIndex {
    ...
    fn write_index(&self, index: &AutomationIndex) -> Result<(), AutomationError> {
        let parent = self
            .path
            .parent()
            .ok_or_else(|| AutomationError::Storage("定时任务配置目录不正确".to_string()))?;
...
fn validate_create(input: &AutomationCreateInput) -> Result<(), AutomationError> {
    if input.name.trim().is_empty() {
        return Err(AutomationError::Validation("name 必填".to_string()));
    }
    if input.prompt.trim().is_empty() {
        return Err(AutomationError::Validation("prompt 必填".to_string()));
    }
    validate_schedule(
        &input.schedule_type,
        input.interval_minutes.unwrap_or(10),
        input.time_of_day.as_deref(),
        input.day_of_week,
        input.day_of_month,
        input.scheduled_at,
    ...
    {
        return Err(AutomationError::Validation(
            "仅支持 Pi Agent runtime".to_string(),
        ));
    }
    if input
        .permission_mode
    ...
    {
        return Err(AutomationError::Validation(
            "permissionMode 不正确".to_string(),
        ));
    }
    if input
        .session_mode
        .as_deref()
        .is_some_and(|value| value != "daily" && value != "reuse")
    {
        return Err(AutomationError::Validation(
            "sessionMode 不正确".to_string(),
        ));
    }
    Ok(())
}
...
fn validate_update(target: &Value, input: &AutomationUpdateInput) -> Result<(), AutomationError> {
    ...
    {
        return Err(AutomationError::Validation("name 不能为空".to_string()));
    }
    if input
        .prompt
        .as_deref()
        .is_some_and(|value| value.trim().is_empty())
    {
        return Err(AutomationError::Validation("prompt 不能为空".to_string()));
    }
    if input
        .agent_runtime
        .as_deref()
    ...
    let schedule_type = input
        .schedule_type
        .as_deref()
        .unwrap_or(value_string(target, "scheduleType").unwrap_or("interval"));
    let interval = input.interval_minutes.unwrap_or_else(|| {
        target
            .get("intervalMinutes")
            .and_then(Value::as_u64)
            .unwrap_or(10)
    ...
    let time_of_day = input
        .time_of_day
        .as_deref()
        .or_else(|| value_string(target, "timeOfDay"));
    let day_of_week = input
        .day_of_week
        .or_else(|| target.get("dayOfWeek").and_then(Value::as_u64));
    let day_of_month = input
        .day_of_month
        .or_else(|| target.get("dayOfMonth").and_then(Value::as_u64));
    let scheduled_at = input
        .scheduled_at
        .or_else(|| target.get("scheduledAt").and_then(Value::as_u64));
    validate_schedule(
        schedule_type,
        interval,
        time_of_day,
...
fn validate_schedule(
    ...
    if !matches!(
        ...
        return Err(AutomationError::Validation(
            "scheduleType 不正确".to_string(),
        ));
    }
    if interval == 0 {
        return Err(AutomationError::Validation(
            "intervalMinutes 必须大于 0".to_string(),
        ));
    }
    if matches!(schedule_type, "daily" | "weekly" | "monthly") && !valid_time(time_of_day) {
        return Err(AutomationError::Validation(
            "scheduleType=daily/weekly/monthly 时 timeOfDay 必填".to_string(),
        ));
    }
    if schedule_type == "weekly" && day_of_week.filter(|value| *value <= 6).is_none() {
        return Err(AutomationError::Validation(
            "scheduleType=weekly 时 dayOfWeek 必填".to_string(),
        ));
    }
    ...
    {
        return Err(AutomationError::Validation(
            "scheduleType=monthly 时 dayOfMonth 必填".to_string(),
        ));
    }
    if schedule_type == "once" && scheduled_at.filter(|value| *value > 0).is_none() {
        return Err(AutomationError::Validation(
            "scheduleType=once 时 scheduledAt 必填".to_string(),
        ));
    }
    Ok(())
}
...
fn valid_time(value: Option<&str>) -> bool {
    ...
    parts.next().is_none()
        && hours.is_some_and(|hour| hour < 24)
        && minutes.is_some_and(|minute| minute < 60)
}
...
fn apply_update(target: &mut Value, input: AutomationUpdateInput) {
    ...
    if let Some(max_runs) = input.max_runs {
        if max_runs == 0 {
            object.remove("maxRuns");
        } else {
            object.insert("maxRuns".to_string(), json!(max_runs));
        }
    }
...
fn migrate_legacy_fields(automation: &mut Value) -> bool {
    let Some(object) = automation.as_object_mut() else {
        return false;
    };
    let mut changed = false;
    if object.get("agentRuntime").and_then(Value::as_str) != Some("pi") {
        object.insert("agentRuntime".to_string(), json!("pi"));
        changed = true;
    }
    if object.get("sessionMode").and_then(Value::as_str) != Some("reuse") {
        object.insert("sessionMode".to_string(), json!("reuse"));
        changed = true;
    }
    if object
        ...
        .is_some_and(|value| value != "bypassPermissions")
    {
        object.insert("permissionMode".to_string(), json!("bypassPermissions"));
        changed = true;
    }
    if !object.contains_key("runHistory") {
        object.insert("runHistory".to_string(), json!([]));
        changed = true;
    }
    if !object.contains_key("runCount") {
        object.insert("runCount".to_string(), json!(0));
        changed = true;
    }
    if !object.contains_key("consecutiveFailures") {
        object.insert("consecutiveFailures".to_string(), json!(0));
        changed = true;
    }
    changed
}
...
fn next_run_at(
    ...
    if schedule_type == "once" {
        return scheduled_at.unwrap_or(from.saturating_add(600_000));
    }
    if schedule_type == "interval" {
        return from.saturating_add(interval_minutes.max(1).saturating_mul(60_000));
    }
    let (hours, minutes) = parse_time_of_day(time_of_day).unwrap_or((9, 0));
    let Some(mut next) = Local.timestamp_millis_opt(from as i64).single() else {
        return from.saturating_add(600_000);
    };
    ...
    if schedule_type == "daily" {
        if next.timestamp_millis() <= from as i64 {
        ...
        if difference == 0 && next.timestamp_millis() <= from as i64 {
            difference = 7;
        }
        ...
        next = local_datetime(
            year,
            month,
            target_day.min(days_in_month(year, month)),
            hours,
            minutes,
        )
        .unwrap_or(next + chrono::Duration::days(31));
    }
...
fn days_in_month(year: i32, month: u32) -> u32 {
    ...
    next_month
        .single()
        .map(|value| (value - chrono::Duration::days(1)).day())
        .unwrap_or(28)
}
...
fn runnable(channel_id: &str, workspace_id: Option<&str>) -> bool {
    !channel_id.trim().is_empty() && workspace_id.is_some_and(|value| !value.trim().is_empty())
}
fn item_id(value: &Value) -> Option<&str> {
    value.get("id").and_then(Value::as_str)
}
fn created_at(value: &Value) -> u64 {
...
fn set_number(value: &mut Value, key: &str, number: u64) {
    set_value(value, key, json!(number));
}
fn set_bool(value: &mut Value, key: &str, state: bool) {
    set_value(value, key, json!(state));
}
...
fn storage_error(error: std::io::Error) -> AutomationError {
    AutomationError::Storage(format!("定时任务存储失败: {}", error))
}

fn generate_id() -> String {
    let mut bytes = [0_u8; 16];
    if getrandom(&mut bytes).is_err() {
        bytes[..8].copy_from_slice(&now_millis().to_le_bytes());
    }
