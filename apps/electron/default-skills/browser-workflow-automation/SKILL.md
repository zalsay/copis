...
本 Skill 面向绑定 Copis 内部网页页签的主对话。它用于把用户的自然语言目标转换成一组受 AI浏览器授权和敏感操作规则约束的网页操作，不是外部 Skill 的安装或调用说明。

## 触发条件与前置检查

当用户要求“打开某个 URL/页面并继续完成一系列网页操作”，或明确要求运行一个已保存的网页 Workflow 时，使用本 Skill。

开始前确认当前会话已经绑定 Copis 内部 AI浏览器页签：

- 没有 Browser Context 时，说明需要用户先在 Copis 中打开并绑定内部网页页签，然后停止网页控制。
...
3. 在授权模式下，对每个普通动作按“重新观察 → 使用最新 ref → 执行动作”的顺序执行。可用动作工具包括 `BrowserPageClick`、`BrowserPageType`、`BrowserPageSelect`、`BrowserPagePress` 和 `BrowserPageScroll`。
...
询问模式只允许观察和读取页面，不得导航、点击、输入、选择、按键或滚动。只有授权模式才可执行普通网页操作；授权模式也不解除敏感操作和高影响操作的边界。
...
5. Workflow 失败、页面 Origin 不匹配、步骤不再满足条件、遇到敏感步骤或需要人工检查时，立即停止并报告原因和失败步骤；不要猜测点击、自由修复或修改已保存 Workflow。

Workflow 的固定步骤仍受 AI浏览器权限、跨 Origin 导航、高影响动作确认和敏感字段规则约束。Workflow 运行结果也必须在结束后读取并核对当前页面状态。

## 安全边界

- 密码、验证码、一次性验证码、支付信息、Captcha、`secret` 字段和文件上传必须由用户亲自填写、选择或提交。不要代填、代读、代点验证，也不要要求用户把敏感值发到对话中。
- 用户已经选择授权模式后，普通点击以及高风险点击、选择和按键（包括 `Enter`）按当前页面授权直接执行；即使目标是发送、提交、购买或删除，也不重复请求单次审批。仍需先 Observe、使用最新 `ref`，并且只执行用户明确要求的目标。
- 只有跨 Origin 的 `BrowserPageNavigate` 仍需要一次单独审批；审批通过后才能导航，导航后按现有页面授权状态重新处理。
