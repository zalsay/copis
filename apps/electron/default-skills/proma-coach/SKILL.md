...
- **CLAUDE.md** —— Proma 工作区管理的项目规则、硬约束和关键入口；只按系统提示给出的绝对路径读取和维护
...
- **模式选择**：这事该用 Agent 还是 Chat、要不要做工具、Skill 和工具怎么分、SubAgent/CLAUDE.md/Proma 工作区规则应该放哪。
...
- 按需搜索系统提示给出的 Proma 工作区 CLAUDE.md、Auto Memory 与 Skills，以及会话级 Context、项目级 Context 和相关本地文档——是否其实已有相关沉淀但没被使用？
...
1. 读系统提示给出的 Proma 工作区 CLAUDE.md 前 30 行和目录结构
2. 读系统提示给出的 Proma 工作区 Auto Memory 索引（**MEMORY.md 只放主题索引和路由，每条最多 1-2 句摘要；详细内容一律拆到同目录或子目录下的主题文件**）
...
> CLAUDE.md 是**Proma 工作区管理的项目规范**——以系统提示给出的 `Proma 工作区 CLAUDE.md` 绝对路径为准，记录项目硬规则和入口。Skill 是**可复用的流程**——跨项目可用，按需触发。规则放 CLAUDE.md，流程做 Skill。
...
2. **分类**：
   ...
   - 这条偏好会影响未来输出风格，应写入 Memory；如果 Proma 工作区 CLAUDE.md 已有输出风格规则，也可以只留一个简短入口。
3. **主动设计维护建议**：
...
4. **闭环**：用户确认后，小幅更新 Memory。给出一句话总结："我已把这条偏好写入 Proma 工作区记忆，下次类似输出会默认避开 emoji；如果哪天这条偏好改变，我们再修订这条记忆。"
