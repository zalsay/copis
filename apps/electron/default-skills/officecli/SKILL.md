...
Copis Electron is the sole owner of all functional module lifecycle operations.
Use the `功能模块` section in Copis `关于/更新` settings to install or update
OfficeCLI. Electron resolves the release, verifies `SHA256SUMS`, stores the
binary in its local module cache, activates one version, and injects the active
binary into the Agent process `PATH`.

This Skill only describes how to use the already activated `officecli` command.
It must not download, install, update, or modify `PATH` for OfficeCLI. If the
command is unavailable, ask the user to install the OfficeCLI functional module
from Copis settings.

Use `officecli --version` only to verify that Electron has made the active
module available.
