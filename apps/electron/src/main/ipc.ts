...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, runtime: AgentRuntime): Promise<AgentSessionMeta> => {
      ...
      // 历史会话缺失 runtime 时按 Pi 迁移。
      const previousRuntime: AgentRuntime = isAgentRuntime(current.agentRuntime) ? current.agentRuntime : 'pi'
      ...
      if (previousRuntime !== runtime) {
        // Pi session artifact 不可与未知的历史 runtime 互用；下一轮从 Copis 已持久化的转录恢复。
  ...
  const validateAutomationRuntimePolicy = (
    ...
    if (input.agentRuntime !== undefined && input.agentRuntime !== 'pi') {
      throw new Error('Copis 仅支持 Pi Agent runtime')
    }
  }
