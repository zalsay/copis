...
import { SCREENSHOT_LIMITS } from '@copis/shared'
...
.copis-screenshot-wrapper{padding:${SCREENSHOT_PADDING_TOP}px ${SCREENSHOT_PADDING_X}px;background:${bg};width:max-content;max-width:100%;margin:0 auto}
.copis-screenshot-sheet{width:max-content;max-width:100%;margin:0 auto;background:${bg}}
.copis-screenshot-sheet [contenteditable],
.copis-screenshot-sheet [contenteditable="false"]{outline:none}
.copis-screenshot-sheet .ProseMirror-selectednode,
.copis-screenshot-sheet .selectedCell::after{display:none!important}
</style></head><body class="${safeThemeClass}">
<div class="copis-screenshot-wrapper">
<main class="copis-screenshot-sheet">${safeHtml}</main>
</div>
</body></html>`
}
...
async function screenshotCapture(htmlContent: string, width: number): Promise<Buffer> {
  const tmpPath = join(tmpdir(), `copis-ss-${Date.now()}.html`)
  ...
  return withLock(async () => {
    try {
      ...
      const { canceled, filePath } = await dialog.showSaveDialog({
        title: '保存截图',
        defaultPath: join(homedir(), 'Desktop', `copis-${ts}.png`),
