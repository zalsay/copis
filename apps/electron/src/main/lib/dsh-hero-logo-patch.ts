...
const HERO_LOGO_MARKER = 'className: "copis-hero-logo-light"'

function createHeroFishReplacement(lightLogoSvg: string): string {
  const lightLogoDataUri = `data:image/svg+xml;base64,${Buffer.from(lightLogoSvg).toString('base64')}`

  return `\t\tfunction HeroFish({ hovering }) {
...
\t\t\t\t\t\tclassName: "copis-hero-logo-light",
\t\t\t\t\t\thref: "${lightLogoDataUri}",
\t\t\t\t\t\tx: 347,
\t\t\t\t\t\ty: 347,
\t\t\t\t\t\twidth: 560,
\t\t\t\t\t\theight: 560,
...
\t\t\t\t\t\tclassName: "copis-hero-logo-dark",
\t\t\t\t\t\tchildren: [
...
const HERO_FISH_PATTERN = /(?:\t*)function HeroFish\(\{\s*hovering\s*\}\)\s*\{[\s\S]*?\n\t*\}\n(?=\t*(?:\/\*\*[\s\S]*?)?function HeroShell)/

/**
 * 修改 DSH 创造模式英雄区标语「探索未至之境」前的 Logo：
 * 浅色主题使用深色金属 Logo，深色主题保留银色双环 Logo，并保持悬停放大动效。
 */
export function patchDshHeroLogoSource(source: string, lightLogoSvg: string): string {
  if (source.includes(HERO_LOGO_MARKER)) {
    return source
  }

  if (!HERO_FISH_PATTERN.test(source)) {
    throw new Error('DSH 创造模式 HeroFish 组件未找到匹配结构')
  }

  return source.replace(HERO_FISH_PATTERN, `${createHeroFishReplacement(lightLogoSvg)}\n`)
}

/** 构建期及已安装模块启动前共用；确保 DSH 探索未至之境标语前的 Logo 更新为新版。 */
export function patchDshHeroLogoRuntime(runtimeRoot: string, lightLogoPath: string): boolean {
  const entrypoint = join(runtimeRoot, CLIENT_ENTRY)
  if (!existsSync(entrypoint)) return false
  const source = readFileSync(entrypoint, 'utf8')
  const lightLogoSvg = readFileSync(lightLogoPath, 'utf8')
  const patched = patchDshHeroLogoSource(source, lightLogoSvg)
