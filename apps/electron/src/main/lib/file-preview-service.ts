...
function getPreviewTmpDir(): string {
  const dir = join(tmpdir(), 'copis-preview')
  if (!existsSync(dir)) {
...
export async function preparePdfPreview(filePath: string, basePaths?: string[]): Promise<{ resolvedPath: string; tmpHtmlUrl: string } | null> {
  ...
  try {
    const { registerCopisDirectoryPath, registerCopisFilePath } = await import('./local-file-protocol')
    registerFilePath = registerCopisFilePath
    fileUrl = registerCopisFilePath(safePath)
    pdfScriptUrl = registerCopisFilePath(require.resolve(`${PDFJS_PACKAGE}/build/pdf.min.mjs`))
    pdfWorkerUrl = registerCopisFilePath(require.resolve(`${PDFJS_PACKAGE}/build/pdf.worker.min.mjs`))
    const pdfPackageDir = dirname(require.resolve(`${PDFJS_PACKAGE}/package.json`))
    standardFontDataUrl = `${registerCopisDirectoryPath(join(pdfPackageDir, 'standard_fonts'))}/`
  } catch (err) {
