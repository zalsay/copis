...
describe('Browser Agent 自动 Skill 引用', () => {
  test('有 Browser Context 时自动加入网页控制 Skill', () => {
  ...
  test('没有 Browser Context 时不注入网页控制 Skill', () => {
  ...
  test('显式引用同名 Skill 时不重复添加', () => {
    expect(resolveBrowserAgentSkillMentions([BROWSER_PAGE_CONTROL_SKILL], true)).toEqual([
      BROWSER_PAGE_CONTROL_SKILL,
    ])
  })
  ...
  test('Browser Agent 拒绝进入或退出计划模式，普通 Agent 保持原策略', () => {
    expect(getBrowserAgentPlanToolDenial('EnterPlanMode', true)).toContain('Browser Agent')
    expect(getBrowserAgentPlanToolDenial('ExitPlanMode', true)).toContain('Browser Agent')
