...
export const BROWSER_WORKFLOW_AUTOMATION_SKILL = 'browser-workflow-automation'

const BROWSER_PLAN_TOOL_DENIAL_MESSAGE = 'AI浏览器不支持计划模式，网页控制请使用 Browser Page 工具及其网页授权流程。'

/** AI浏览器不受工作区只读默认值影响，始终以完全自动模式运行。 */
...
/** AI浏览器不参与 Copis 的 EnterPlanMode/ExitPlanMode 审批链。 */
...
export function resolveBrowserAgentSkillMentions(
  ...
  for (const requiredSkill of [BROWSER_PAGE_CONTROL_SKILL, BROWSER_WORKFLOW_AUTOMATION_SKILL]) {
    if (!effectiveSkills.includes(requiredSkill)) effectiveSkills.push(requiredSkill)
  }
