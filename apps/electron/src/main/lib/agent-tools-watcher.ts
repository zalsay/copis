...
export function startAgentToolsWatcher(): void {
  ...
  if (!existsSync(filePath)) {
    console.log('[Agent 工具监听] 配置文件不存在，跳过:', filePath)
    return
  }
  ...
  try {
    watcher = watch(filePath, (_eventType) => {
      ...
      debounceTimer = setTimeout(() => {
        ...
        console.log('[Agent 工具监听] 配置变更，已通知渲染进程')
        debounceTimer = null
      }, DEBOUNCE_MS)
    })
    ...
    watcher.on('error', (err) => {
      console.error('[Agent 工具监听] 运行时错误，关闭监听:', err)
      try { watcher?.close() } catch { /* 已关闭 */ }
      watcher = null
    })

    console.log('[Agent 工具监听] 已启动')
  } catch (err) {
    console.error('[Agent 工具监听] 启动失败:', err)
  }
}
