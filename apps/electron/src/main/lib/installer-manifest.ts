 ...
 *
 * 从 copis-api 的 /api/v1/installers/manifest 接口拉取第三方安装包清单，
 * 带 5 分钟缓存和内置 fallback——断网或接口不可用时至少能拿到官方上游 URL。
 */

import type { InstallerManifest, InstallerSource } from '@copis/shared'

// 服务端接口域名属于既有外部契约，品牌改名不改变它。
const COPIS_API_BASE = 'https://api.proma.cool'
const MANIFEST_URL = `${COPIS_API_BASE}/api/v1/installers/manifest`
