 ...
 * HTTP 工具执行器（自定义工具用）
 *
 * 根据 AgentToolMeta 中的 httpConfig 配置执行 HTTP 请求。
 * 支持 URL/Body 模板占位符替换、超时控制、响应路径提取。
 */

import type { ToolCall, ToolResult } from '@copis/core'
import type { AgentToolMeta, AgentToolHttpConfig } from '@copis/shared'
 ...
 * @returns 工具执行结果
 */
export async function executeHttpTool(toolCall: ToolCall, meta: AgentToolMeta): Promise<ToolResult> {
