...
export const BROWSER_WORKFLOW_RUN_DESCRIPTION = '按 Browser Workflow 执行跨页面自动化，缺省执行最新版本；也可显式指定特定版本号。已批准版本由 Copis 主进程校验 Playwright 脚本摘要后，通过 Playwright 连接当前 Electron Chromium 执行；Agent 只能调用 BrowserWorkflowRun，不得通过 bash、Node.js 或其他工具直接运行、修改或重新生成 Workflow Playwright 脚本。用户运行失败时会保留失败页面给当前 Browser Agent 重新观察；无人值守和委派运行只记录失败，不会动态重规划。'
