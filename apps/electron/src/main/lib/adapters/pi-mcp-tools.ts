 ...
 *
 * Pi SDK 当前没有直接的 mcpServers 参数，因此 Copis 在主进程连接用户配置的 MCP server，
 * 并把 MCP tools 映射成 Pi customTools。
 */
...
class PiMcpClientManager {
  ...
  private async createConnection(serverName: string, config: PiMcpServerConfig, key: string): Promise<McpConnection> {
    ...
    const client = new Client({ name: 'copis-pi-agent-mcp-bridge', version: '0.1.0' }, { capabilities: {} })
 ...
 *
 * 本函数仅供 Pi runtime 使用，负责将连接好的 MCP tools 转换为 Pi customTools。
 */
