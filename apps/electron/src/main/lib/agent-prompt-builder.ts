...
function buildWorkspacePromptPaths(workspaceSlug: string, sessionId: string, agentCwd?: string) {
  ...
  const isInvestmentWorkspace = workspaceSlug === 'investment'
...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  ...
  const isInvestmentWorkspace = workspacePaths?.isInvestmentWorkspace === true
...
- 项目来源目录: ${workspacePaths?.projectSourceRoot}（${isInvestmentWorkspace ? '固定投资工作区的 Investment 根目录' : workspacePaths?.isLocalProject ? '用户选择的本地文件夹，可读取已有资料' : 'Copis 托管的工作区来源目录'}）
- 项目开发目录: ${workspacePaths?.projectRoot}（用于可运行项目代码、依赖配置和启动脚本；用户新建项目统一放在这里）
- 默认产物输出目录: ${workspaceWriteRoot}（普通产物（报告、导出文件、附件等）默认写入该目录；生成的图片和其他交付物也放在这里；${isInvestmentWorkspace ? 'Investment 根目录本身可写，不要额外创建 copis/ 或 project/ 包装目录' : '不要默认写入项目来源根目录'}）
- Agent 可写目录: ${workspaceWriteRoot}、${workspacePaths?.projectRoot}（${isInvestmentWorkspace ? 'Investment 根目录直接用于工作与输出' : '项目来源目录保持只读'}）
...
- **项目级** \`${workspaceContextDir}\`：跨会话共享的持久文档，存放长期 note.md、项目级知识等；长期 Context 继续写入项目级 Context 的绝对路径 \`${workspaceContextDir}\`${isInvestmentWorkspace ? '。投资工作区优先复用已有 copis/.context；若无旧目录，则使用 Investment 根下的 .context。' : '，也就是对应 Copis 目录下的 `.context/`（来源目录保持只读时仍使用来源目录下的 `copis/.context`）'}
...
${isInvestmentWorkspace ? '- Investment 来源根同时是默认可写工作目录；直接在其中组织所需文件，不要再自动套建 copis/ 或 project/ 目录' : '- 项目来源目录只作为参考；新项目代码、依赖配置和启动脚本仍写入项目开发目录，不要把它当作可随意清理的临时目录'}`)
  }
...
5. **智能项目定位**：
   - 若用户未显式指定项目，默认在当前绑定项目（${ctx.workspaceName ?? '默认工作区'}）中执行；
