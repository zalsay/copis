...
import type { ToolDefinition, ToolParameterProperty } from '@copis/core'
import type { AgentToolInfo, AgentToolMeta } from '@copis/shared'
...
/**
 * 将 AgentToolMeta 转换为 ToolDefinition（Provider API 格式）
 *
 * 将简化的 params 格式映射为 JSON Schema。
 */
function convertMetaToDefinition(meta: AgentToolMeta): ToolDefinition {
 ...
 */
export function getEnabledAgentTools(enabledToolIds?: string[]): EnabledToolsResult {
 ...
 */
export function getAllAgentToolInfos(): AgentToolInfo[] {
  const config = getAgentToolsConfig()
  const infos: AgentToolInfo[] = []
