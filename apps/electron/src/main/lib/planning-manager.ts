...
function hydrateCalendarEvents(rows: CalendarEventRow[]): CalendarEvent[] {
  ...
  return rows.map((row) => ({
    ...
    allDay: row.all_day === 1, status: row.status, completedAt: row.completed_at ?? undefined, groupId: row.calendar_group_id ?? undefined, group: row.calendar_group_id ? groups.get(row.calendar_group_id) : undefined,
...
function calendarEventFromRow(row: CalendarEventRow): CalendarEvent {
  return { id: row.id, title: row.title, notes: row.notes ?? undefined, startAt: row.start_at, endAt: row.end_at ?? undefined, allDay: row.all_day === 1, status: row.status, completedAt: row.completed_at ?? undefined, groupId: row.calendar_group_id ?? undefined, group: getPlanningGroup(row.calendar_group_id, 'calendar'), tags: getTags('calendar_event', row.id), reminders: getReminders('calendar_event', row.id), workspaceId: row.workspace_id ?? undefined, todoId: row.todo_id ?? undefined, createdAt: row.created_at, updatedAt: row.updated_at }
}
...
export function createCalendarEvent(input: CreateCalendarEventInput): CalendarEvent {
  ...
  const event = {
    ...
    workspaceId: input.workspaceId || undefined, todoId: input.todoId || undefined, status, completedAt: status === 'completed' ? now : undefined, createdAt: now, updatedAt: now,
  }
