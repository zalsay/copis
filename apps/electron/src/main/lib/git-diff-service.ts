...
import type { ChangedFileEntry, UnstagedChangesResult, UntrackedFileEntry } from '@copis/shared'
import { normalizePathForCompare } from '@copis/shared'
import type { ChangeSource, ChangedFileStatus } from '@copis/shared'
 ...
 *
 * 用于安全校验：worktree 常被放在主仓库之外（如 ~/copis-dev/worktrees/xxx），
 ...
 */
export async function listWorktrees(repoPath: string): Promise<import('@copis/shared').WorktreeInfo[]> {
  ...
  const worktrees: import('@copis/shared').WorktreeInfo[] = []
...
export async function getWorktreeChanges(
...
): Promise<import('@copis/shared').UnstagedChangesResult> {
  ...
  const allFiles: import('@copis/shared').ChangedFileEntry[] = []
  const fileMap = new Map<string, import('@copis/shared').ChangedFileEntry>()
  ...
  if (committedStatus) {
    for (const line of committedStatus.split('\n').filter(Boolean)) {
      ...
      let status: import('@copis/shared').ChangedFileStatus
      let filePath: string
      ...
      const entry: import('@copis/shared').ChangedFileEntry = {
        filePath,
        status,
        additions: stats.additions,
        deletions: stats.deletions,
  ...
  // 3. 新文件（未追踪）
  const untrackedFiles: import('@copis/shared').UntrackedFileEntry[] = []
  const untrackedOutput = await runGitCommand(['ls-files', '--others', '--exclude-standard'], gitRoot)
  if (untrackedOutput) {
    for (const rel of untrackedOutput.split('\n').filter(Boolean)) {
      if (!fileMap.has(rel)) {
