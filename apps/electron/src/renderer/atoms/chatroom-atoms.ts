...
export const chatRoomApplyEventAtom = atom(null, (get, set, event: ChatRoomEventEnvelope) => {
  ...
  if (event.type === 'message.created') {
    ...
    set(chatRoomInvocationsAtom, (current) => { const next = new Map(current); const room = new Map(current.get(roomId) ?? []); const old = room.get(invocationId); const oldIsTerminal = old?.status === 'completed' || old?.status === 'failed' || old?.status === 'rejected'; const incomingStatus = event.type === 'agent.delta' ? 'running' : event.type === 'agent.completed' ? 'completed' : 'failed'; if (oldIsTerminal && (event.type === 'agent.delta' || incomingStatus !== old.status)) return current; const status = oldIsTerminal ? old.status : incomingStatus; room.set(invocationId, { ...(old ?? {}), invocationId, roomId, traceId: typeof payload.traceId === 'string' ? payload.traceId : old?.traceId ?? '', targetAgentId: typeof payload.targetAgentId === 'string' ? payload.targetAgentId : old?.targetAgentId ?? '', triggerMessageId: typeof payload.triggerMessageId === 'string' ? payload.triggerMessageId : old?.triggerMessageId ?? '', depth: typeof payload.depth === 'number' ? payload.depth : old?.depth ?? 0, status, ...(event.type === 'agent.delta' && !oldIsTerminal && typeof payload.delta === 'string' ? { delta: `${old?.delta ?? ''}${payload.delta}` } : {}), ...(typeof payload.failureCode === 'string' ? { failureCode: payload.failureCode } : {}) }); next.set(roomId, room); return next })
  }
})
