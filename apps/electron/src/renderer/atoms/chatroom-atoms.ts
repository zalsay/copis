...
export const chatRoomApplyEventAtom = atom(null, (get, set, event: ChatRoomEventEnvelope) => {
  ...
  if (event.type === 'message.created') {
    ...
    set(chatRoomInvocationsAtom, (current) => {
      ...
      const incomingStatus = event.type === 'agent.delta' ? 'running' : event.type === 'agent.completed' ? 'completed' : 'failed'
      if (oldIsTerminal && (event.type === 'agent.delta' || incomingStatus !== old.status)) return current
      const status = oldIsTerminal ? old.status : incomingStatus
