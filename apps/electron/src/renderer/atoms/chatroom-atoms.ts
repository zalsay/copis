...
export type ChatRoomSendState = { roomId: string; clientMessageId: string; status: 'sending' | 'sent' | 'failed'; error?: string }
...
export const chatRoomSendMessageAtom = atom(null, async (_get, set, input: { api: { sendMessage(value: { roomId: string; content: string; mentionAgentIds: string[]; attachmentIds: string[]; clientMessageId: string }): Promise<ChatRoomMessage> }; roomId: string; content: string; mentionAgentIds: string[]; attachmentIds: string[]; clientMessageId?: string }) => { const clientMessageId = input.clientMessageId ?? crypto.randomUUID(); set(chatRoomSendStatesAtom, (current) => new Map(current).set(clientMessageId, { roomId: input.roomId, clientMessageId, status: 'sending' })); try { const result = await input.api.sendMessage({ roomId: input.roomId, content: input.content, mentionAgentIds: input.mentionAgentIds, attachmentIds: input.attachmentIds, clientMessageId }); set(chatRoomSendStatesAtom, (current) => new Map(current).set(clientMessageId, { roomId: input.roomId, clientMessageId, status: 'sent' })); return result } catch (error) { set(chatRoomSendStatesAtom, (current) => new Map(current).set(clientMessageId, { roomId: input.roomId, clientMessageId, status: 'failed', error: error instanceof Error ? error.message : '发送失败' })); throw error } })
export const chatRoomRetrySendAtom = atom(null, async (_get, set, input: { api: { sendMessage(value: { roomId: string; content: string; mentionAgentIds: string[]; attachmentIds: string[]; clientMessageId: string }): Promise<ChatRoomMessage> }; roomId: string; content: string; mentionAgentIds: string[]; attachmentIds: string[]; clientMessageId: string }) => { set(chatRoomSendStatesAtom, (current) => new Map(current).set(input.clientMessageId, { roomId: input.roomId, clientMessageId: input.clientMessageId, status: 'sending' })); try { const result = await input.api.sendMessage(input); set(chatRoomSendStatesAtom, (current) => new Map(current).set(input.clientMessageId, { roomId: input.roomId, clientMessageId: input.clientMessageId, status: 'sent' })); return result } catch (error) { set(chatRoomSendStatesAtom, (current) => new Map(current).set(input.clientMessageId, { roomId: input.roomId, clientMessageId: input.clientMessageId, status: 'failed', error: error instanceof Error ? error.message : '发送失败' })); throw error } })
