...
describe('chatRoomApi', () => {
  ...
  test('创建请求只发送 name/shareCode，不携带 Agent 配置', async () => { let body = ''; const api = createChatRoomApi({ baseUrl: 'http://test', fetchImpl: async (_url, init) => { body = String(init?.body); return response({ roomId: 'r1', name: 'R' }) } }); await api.createRoom({ name: 'R', shareCode: 'AB12' }, ['agent']); expect(JSON.parse(body)).toEqual({ name: 'R', shareCode: 'AB12' }) })
