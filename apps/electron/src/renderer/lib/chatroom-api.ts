...
export function createChatRoomApi(options: { fetchImpl?: FetchLike; baseUrl?: string } = {}): ChatRoomApi {
  ...
  return {
    ...
    createRoom: async (input, selectedAgents = []) => { const shareCode = normalizeChatRoomShareCode(input.shareCode); if (!shareCode) throw new ChatRoomApiError('分享码必须是4位字母或数字', 400, 'invalid_share_code'); const room = await request('/api/chatrooms/v2/rooms', { method: 'POST', body: json({ name: input.name, shareCode }) }, (v) => normalizeSummary(record(v).room ?? v)); const provision = (globalThis as typeof globalThis & { window?: Window }).window?.electronAPI?.chatrooms?.provisionAgent; if (provision) { for (const agent of selectedAgents.slice(0, 3)) await provision({ roomId: room.roomId, sourceWorkspaceId: agent.sourceWorkspaceId, displayName: agent.displayName, channelId: agent.channelId, ...(agent.modelId === undefined ? {} : { modelId: agent.modelId }), contextMessageCount: agent.contextMessageCount, memorySharingEnabled: agent.memorySharingEnabled, skillSharingEnabled: agent.skillSharingEnabled }) } return room },
