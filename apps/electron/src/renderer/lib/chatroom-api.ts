import type { ChatRoomAgent, ChatRoomAttachment, ChatRoomCreateInput, ChatRoomEventEnvelope, ChatRoomJoinInput, ChatRoomMember, ChatRoomMessage, ChatRoomSendMessageInput, ChatRoomSummary } from '@copis/shared'
...
function normalizeMessage(value: unknown): ChatRoomMessage { const o = record(value); return { messageId: text(pick(o, 'messageId', 'message_id', 'id')), roomId: text(pick(o, 'roomId', 'room_id')), seq: num(o.seq), senderType: ['user', 'agent', 'system'].includes(text(pick(o, 'senderType', 'sender_type'))) ? pick(o, 'senderType', 'sender_type') as ChatRoomMessage['senderType'] : 'system', senderId: text(pick(o, 'senderId', 'sender_id')), content: text(o.content), mentionAgentIds: array(pick(o, 'mentionAgentIds', 'mention_agent_ids')).filter((v): v is string => typeof v === 'string'), attachmentIds: array(pick(o, 'attachmentIds', 'attachment_ids')).filter((v): v is string => typeof v === 'string'), clientMessageId: text(pick(o, 'clientMessageId', 'client_message_id')), ...(typeof pick(o, 'traceId', 'trace_id') === 'string' ? { traceId: text(pick(o, 'traceId', 'trace_id')) } : {}), ...(typeof pick(o, 'parentMessageId', 'parent_message_id') === 'string' ? { parentMessageId: text(pick(o, 'parentMessageId', 'parent_message_id')) } : {}), depth: num(o.depth), createdAt: text(pick(o, 'createdAt', 'created_at')) } }
...
export interface ChatRoomApi {
  ...
  createRoom(input: ChatRoomCreateInput, selectedAgentIds?: string[]): Promise<ChatRoomSummary>
...
export function createChatRoomApi(options: { fetchImpl?: FetchLike; baseUrl?: string } = {}): ChatRoomApi {
  ...
  return {
    ...
    createRoom: async (input, selectedAgentIds = []) => { const shareCode = normalizeChatRoomShareCode(input.shareCode); if (!shareCode) throw new ChatRoomApiError('分享码必须是4位字母或数字', 400, 'invalid_share_code'); const room = await request('/api/chatrooms/v2/rooms', { method: 'POST', body: json({ name: input.name, shareCode }) }, (v) => normalizeSummary(record(v).room ?? v)); const provision = (globalThis as typeof globalThis & { window?: Window }).window?.electronAPI?.chatrooms?.provisionAgent; if (provision) { for (const agentId of selectedAgentIds.slice(0, 3)) await provision({ roomId: room.roomId, sourceWorkspaceId: agentId, displayName: agentId, channelId: agentId, memorySharingEnabled: false, skillSharingEnabled: false }) } return room },
    ...
    getMessages: (roomId, options = {}) => { const qs = new URLSearchParams(); if (options.beforeSeq !== undefined) qs.set('beforeSeq', String(options.beforeSeq)); if (options.limit !== undefined) qs.set('limit', String(options.limit)); return request(`/api/chatrooms/v2/rooms/${encodeURIComponent(roomId)}/messages${qs.size ? `?${qs}` : ''}`, {}, (v) => { const o = record(v); return { messages: array(o.messages ?? o.data ?? v).map(normalizeMessage), ...(typeof o.cursor === 'number' ? { cursor: o.cursor } : {}) } }) },
    sendMessage: (input) => request(`/api/chatrooms/v2/rooms/${encodeURIComponent(input.roomId)}/messages`, { method: 'POST', body: json({ content: input.content, mentionAgentIds: input.mentionAgentIds, attachmentIds: input.attachmentIds, clientMessageId: input.clientMessageId ?? crypto.randomUUID() }) }, (v) => normalizeMessage(record(v).message ?? v)),
