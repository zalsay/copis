  ...
  private async connect(controller: AbortController): Promise<void> {
    ...
    try {
      ...
      if (hadConnected) {
        ...
        try { await this.recoverRooms(controller) } catch { recoveryFailed = true; recoveryFailure = true; this.status('reconnecting'); controller.abort(); throw new Error('聊天室事件恢复失败') }
