...
export class ChatRoomSseClient {
  ...
  private async connect(controller: AbortController): Promise<void> {
    ...
    try {
      ...
      if (hadConnected) { this.liveBuffering = true; try { await this.recoverRooms(controller) } catch { this.status('reconnecting') } finally { this.liveBuffering = false; const queued = this.liveBuffer.splice(0); for (const event of queued) this.receive(event, controller) } }
      await livePromise
  ...
  private async recoverRoom(roomId: string, controller: AbortController): Promise<void> { if (this.recovering.has(roomId)) return; this.recovering.add(roomId); try { let afterSeq = this.cursors.get(roomId) ?? 0; for (let page = 0; page < 100; page += 1) { if (controller.signal.aborted) return; const response = await this.fetchImpl(`${this.baseUrl}/api/chatrooms/v2/rooms/${encodeURIComponent(roomId)}/events?afterSeq=${afterSeq}&limit=500`, withHttpApiWebToken({ headers: { Accept: 'application/json' }, signal: controller.signal })); if (!response.ok) throw new Error(`聊天室事件补拉失败（${response.status}）`); const raw = await response.json() as unknown; const root = typeof raw === 'object' && raw !== null ? raw as Record<string, unknown> : {}; const values = Array.isArray(root.data) ? root.data : Array.isArray(root.events) ? root.events : Array.isArray(raw) ? raw : []; if (!values.length) break; let pageLast = afterSeq; for (const value of values) { const event = this.normalizeRecoveryEvent(value, roomId); if (!event) continue; if ((event.seq ?? 0) > pageLast) pageLast = event.seq!; const cursor = this.cursors.get(roomId) ?? 0; if (event.seq! <= cursor) continue; if (cursor > 0 && event.seq! > cursor + 1) throw new Error('聊天室事件恢复出现序列缺口'); this.receive(event, controller) } if (pageLast <= afterSeq || values.length < 500) break; afterSeq = pageLast } const pending = (this.buffered.get(roomId) ?? []).sort((a, b) => (a.seq ?? 0) - (b.seq ?? 0)); this.buffered.delete(roomId); for (const event of pending) this.receive(event, controller) } finally { this.recovering.delete(roomId) } }
