...
export class ChatRoomSseClient {
  ...
  private liveBuffering = false
  private readonly liveBuffer: ChatRoomEventEnvelope[] = []
  ...
  private async connect(controller: AbortController): Promise<void> {
    ...
    try {
      ...
      const readLive = async (): Promise<void> => { while (!controller.signal.aborted) { const part = await reader.read(); if (part.done) break; buffer += decoder.decode(part.value, { stream: true }); const frames = buffer.split(/\r?\n\r?\n/); buffer = frames.pop() ?? ''; for (const frame of frames) { const event = parseChatRoomSseFrame(frame); if (event) { if (this.liveBuffering) this.liveBuffer.push(event); else this.receive(event, controller) } } } }
      const livePromise = readLive()
      if (hadConnected) {
        this.liveBuffering = true
        let recoveryFailed = false
        try { await this.recoverRooms(controller) } catch { recoveryFailed = true; recoveryFailure = true; this.failConnection(controller); throw new Error('聊天室事件恢复失败') }
        finally { this.liveBuffering = false; const queued = this.liveBuffer.splice(0); if (!recoveryFailed) for (const event of queued) this.receive(event, controller) }
      }
      await livePromise
  ...
  private async recoverRoom(roomId: string, controller: AbortController): Promise<void> { if (this.recovering.has(roomId)) return; this.recovering.add(roomId); try { let afterSeq = this.cursors.get(roomId) ?? 0; let exhausted = true; for (let page = 0; page < 100; page += 1) { if (controller.signal.aborted || !this.roomIds.has(roomId)) return; const response = await this.fetchImpl(`${this.baseUrl}/api/chatrooms/v2/rooms/${encodeURIComponent(roomId)}/events?afterSeq=${afterSeq}&limit=500`, withHttpApiWebToken({ headers: { Accept: 'application/json' }, signal: controller.signal })); if (!response.ok) throw new Error(`聊天室事件补拉失败（${response.status}）`); const raw = await response.json() as unknown; const root = typeof raw === 'object' && raw !== null ? raw as Record<string, unknown> : {}; const values = Array.isArray(root.data) ? root.data : Array.isArray(root.events) ? root.events : Array.isArray(raw) ? raw : []; if (!values.length) { exhausted = false; break } let pageLast = afterSeq; for (const value of values) { const event = this.normalizeRecoveryEvent(value, roomId); if (!event) throw new Error('聊天室事件恢复响应包含无效事件'); if ((event.seq ?? 0) > pageLast) pageLast = event.seq!; const cursor = this.cursors.get(roomId) ?? 0; if (event.seq! <= cursor) continue; if (event.seq! > cursor + 1) throw new Error('聊天室事件恢复出现序列缺口'); this.receive(event, controller) } if (pageLast <= afterSeq) throw new Error('聊天室事件恢复序列未前进'); if (values.length < 500) { exhausted = false; break } afterSeq = pageLast } if (exhausted) throw new Error('聊天室事件恢复达到分页上限，请重新同步'); const pending = (this.buffered.get(roomId) ?? []).sort((a, b) => (a.seq ?? 0) - (b.seq ?? 0)); this.buffered.delete(roomId); for (const event of pending) this.receive(event, controller) } finally { this.recovering.delete(roomId) } }
  private receive(event: ChatRoomEventEnvelope, controller: AbortController): void { const roomId = event.roomId; if (roomId && !this.roomIds.has(roomId)) return; if (!roomId || event.seq === undefined) { for (const listener of this.listeners) listener(event); return } const cursor = this.cursors.get(roomId) ?? 0; if (event.seq <= cursor) return; if (event.seq > cursor + 1) { const list = this.buffered.get(roomId) ?? []; list.push(event); this.buffered.set(roomId, list); void this.recoverRoom(roomId, controller).then(() => { const pending = (this.buffered.get(roomId) ?? []).sort((a, b) => (a.seq ?? 0) - (b.seq ?? 0)); this.buffered.delete(roomId); for (const item of pending) this.receive(item, controller) }).catch(() => this.failConnection(controller, roomId)); return } this.cursors.set(roomId, event.seq); for (const listener of this.listeners) listener(event) }
  private failConnection(controller: AbortController, roomId?: string): void { if (this.stopped || this.controller !== controller) return; if (roomId) this.buffered.delete(roomId); else this.buffered.clear(); this.liveBuffer.length = 0; controller.abort(); this.controller = undefined; this.scheduleReconnect() }
