...
export function parseChatRoomSseFrame(frame: string): ChatRoomEventEnvelope | undefined {
  ...
  try {
    ...
    if (typeof parsed !== 'object' || parsed === null || typeof parsed.type !== 'string' || !parsed.type) return undefined
    return {
      type: parsed.type,
      roomId: typeof parsed.roomId === 'string' ? parsed.roomId : undefined,
      seq: typeof parsed.seq === 'number' ? parsed.seq : undefined,
      latestSeq: typeof parsed.latestSeq === 'number' ? parsed.latestSeq : undefined,
      ...(parsed.type === 'local.status' && typeof parsed.code === 'string' ? { code: parsed.code } : {}),
...
export class ChatRoomSseClient {
  ...
  constructor(options: { fetchImpl?: FetchLike; baseUrl?: string } = {}) {
    const fetchImpl = options.fetchImpl ?? fetch
    this.fetchImpl = (input, init) => fetchImpl.call(globalThis, input, init)
    this.baseUrl = options.baseUrl ?? RENDERER_HTTP_API_BASE_URL
  }
  ...
  private async connect(controller: AbortController, generation: number): Promise<void> {
    ...
    try {
      ...
      this.attempt = 0; const hadConnected = this.hasConnected; this.hasConnected = true; const reader = response.body.getReader(); const decoder = new TextDecoder(); let buffer = ''
      ...
      if ((!controller.signal.aborted || recoveryFailure) && !this.stopped) this.scheduleReconnect()
    }
  ...
  private async fetchTerminalInvocations(roomId: string, controller: AbortController, generation: number): Promise<void> {
    ...
    while (true) {
      ...
      if (response.status === 403 || response.status === 404) {
        ...
        if (this.isCurrent(controller, generation) && this.roomIds.has(roomId)
          && (code === 'not_found' || code === 'not_member' || code === 'forbidden')) {
          this.dispatch({ type: 'local.status', roomId, code: code === 'not_found' ? 'room_not_found' : 'not_member', payload: null })
          return
        }
      }
  ...
  private receive(event: ChatRoomEventEnvelope, controller: AbortController, generation: number): void {
    if (!this.isCurrent(controller, generation)) return
    const roomId = event.roomId
    if (roomId && !this.roomIds.has(roomId)) return
    if (event.type === 'local.status' && !roomId) {
      if (event.code === 'realtime_connected') this.status('connected')
      else if (event.code === 'not_authenticated' || event.code === 'auth_expired') this.status('auth_expired')
      else if (['realtime_disconnected', 'realtime_unavailable', 'not_found', 'not_member', 'forbidden', 'bridge_unavailable'].includes(event.code ?? '')) this.status('reconnecting')
    }
    if (event.type === 'room.recovery_ready' && roomId) {
      void this.recoverTerminalInvocations(roomId, controller, generation, true)
        .catch(() => this.failConnection(controller, generation, roomId))
    }
    if (!roomId || event.seq === undefined) { this.dispatch(event); return }
    const cursor = this.cursors.get(roomId) ?? 0
    if (event.seq <= cursor) return
    if (event.seq > cursor + 1) {
      const list = this.buffered.get(roomId) ?? []
      list.push(event)
      this.buffered.set(roomId, list)
      void this.recoverRoom(roomId, controller, generation).then(() => {
        if (!this.isCurrent(controller, generation)) return
        const pending = (this.buffered.get(roomId) ?? []).sort((a, b) => (a.seq ?? 0) - (b.seq ?? 0))
        this.buffered.delete(roomId)
        for (const item of pending) this.receive(item, controller, generation)
      }).catch(() => this.failConnection(controller, generation, roomId))
      return
    }
    this.cursors.set(roomId, event.seq)
    this.dispatch(event)
  }
