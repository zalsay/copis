...
import { COPIS_STARTER_FEATURES, COPIS_BROWSER_STARTER_FEATURES } from './NewSessionFeatureChips'
...
describe('NewSessionFeatureChips', () => {
  ...
  test('每个入口都具备完整 title, description 与可执行 prompt', () => {
    for (const feature of [...COPIS_STARTER_FEATURES, ...COPIS_BROWSER_STARTER_FEATURES]) {
