...
test('图片生成的列表描述使用用户友好的内置服务文案', () => {
  expect(imageServer?.description).toBe('使用 Copis 内置的图片生成服务，登录 Copis Working 后即可生成图片。')
...
test('图片生成详情说明无需额外配置', () => {
  ...
  expect(detailSheetSource).toContain("description: '登录 Copis Working 后即可使用图片生成功能，无需额外配置。'")
