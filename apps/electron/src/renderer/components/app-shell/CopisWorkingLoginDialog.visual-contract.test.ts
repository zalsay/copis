...
describe('Working 登录弹窗视觉契约', () => {
  ...
  test('Given 登录弹窗 When 查看主操作与状态标记 Then 使用 primary 和 primary-foreground', () => {
    ...
    const resetStepRule = ruleBody('\\.copis-working-reset-steps span\\.active')
    ...
    expect(resetStepRule).toContain('border-color: var(--ui-primary)')
    expect(resetStepRule).toContain('background: var(--ui-primary)')
    expect(resetStepRule).toContain('color: var(--ui-primary-foreground)')
