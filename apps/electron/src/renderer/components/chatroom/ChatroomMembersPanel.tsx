...
export function ChatroomMembersPanel({ role, members, agents }: ChatroomMembersPanelProps): React.ReactElement {
  return <aside aria-label="聊天室成员" className="w-64 shrink-0 border-l border-border/50 p-3 overflow-y-auto space-y-4">
    ...
    {role === 'host' && <div className="space-y-1 border-t border-border/40 pt-3"><button type="button" className="w-full rounded-lg bg-muted/50 px-2 py-1.5 text-left text-xs hover:bg-muted">管理 Agent</button><button type="button" className="w-full rounded-lg bg-muted/50 px-2 py-1.5 text-left text-xs hover:bg-muted">聊天室设置</button></div>}
  </aside>
}
