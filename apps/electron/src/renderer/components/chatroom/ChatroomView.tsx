...
import { chatRoomActiveRoomIdAtom, chatRoomClearPermissionRequestAtom, chatRoomConnectionStatusAtom, chatRoomDeletedRoomIdsAtom, chatRoomDetailsAtom, chatRoomInvocationsAtom, chatRoomMarkReadAtom, chatRoomMessagesAtom, chatRoomPermissionRequestsAtom, chatRoomRemoveRoomAtom, chatRoomRoomsAtom } from '@/atoms/chatroom-atoms'
...
export function ChatroomView({ roomId }: ChatroomViewProps): React.ReactElement {
  ...
  const permission = getHostChatRoomPermission(detailsRaw?.room ?? rooms.find((item) => item.roomId === roomId), permissionRequests.values(), roomId)
  ...
  const clearPermissionRequest = useSetAtom(chatRoomClearPermissionRequestAtom)
  ...
  React.useEffect(() => { let alive = true; void Promise.all([chatRoomApi.getRoom(roomId), chatRoomApi.getMessages(roomId, { limit: 100 }), chatRoomApi.listAttachments(roomId)]).then(([detail, history, nextAttachments]) => { if (!alive || deletedRoomIdsRef.current.has(roomId)) return; setDetails((current) => ({ ...current, [roomId]: detail })); setMessages((current) => new Map(current).set(roomId, history.messages)); setAttachments(nextAttachments) }).catch((error) => { if (alive && !deletedRoomIdsRef.current.has(roomId)) console.warn('[聊天室] 加载失败:', error) }); return () => { alive = false } }, [roomId, setDetails, setMessages])
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    {permission && room.role === 'host' && <div role="alert" className="flex items-center gap-2 bg-amber-500/10 px-4 py-2 text-xs">敏感操作：{permission.toolName} · {permission.summary}<button type="button" className="ml-auto underline" onClick={() => void window.electronAPI.chatrooms.respondPermission({ requestId: permission.requestId, behavior: 'allow' }).then(() => clearPermissionRequest(permission.requestId)).catch((error) => setActionError(error instanceof Error ? error.message : '批准失败'))}>批准</button><button type="button" className="underline" onClick={() => void window.electronAPI.chatrooms.respondPermission({ requestId: permission.requestId, behavior: 'deny' }).then(() => clearPermissionRequest(permission.requestId)).catch((error) => setActionError(error instanceof Error ? error.message : '拒绝失败'))}>拒绝</button></div>}
