...
export function getChatRoomMessageClassName(message: Pick<ChatRoomMessage, 'senderType' | 'senderId'>, currentUserId: string | undefined): string {
  if (message.senderType === 'user' && currentUserId === message.senderId) return 'ml-auto bg-primary text-primary-foreground'
  if (message.senderType === 'user') return 'mr-auto bg-muted/70 text-foreground'
  if (message.senderType === 'agent') return 'mr-auto bg-muted'
  return 'mx-auto bg-muted/40 text-muted-foreground'
}
