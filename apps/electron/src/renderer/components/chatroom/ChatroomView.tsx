...
import { chatRoomActiveRoomIdAtom, chatRoomConnectionStatusAtom, chatRoomDeletedRoomIdsAtom, chatRoomDetailsAtom, chatRoomInvocationsAtom, chatRoomMarkReadAtom, chatRoomMessagesAtom, chatRoomRemoveRoomAtom, chatRoomRoomsAtom } from '@/atoms/chatroom-atoms'
...
export function ChatroomView({ roomId }: ChatroomViewProps): React.ReactElement {
  ...
  const [permission, setPermission] = React.useState<{ requestId: string; toolName: string; summary: string } | null>(null)
  const [actionError, setActionError] = React.useState('')
  React.useEffect(() => { if (room.role !== 'host') return; return window.electronAPI.chatrooms.onPermissionRequested((request) => { if (request.roomId === roomId) setPermission(request) }) }, [room.role, roomId])
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    {permission && <div role="alert" className="flex items-center gap-2 bg-amber-500/10 px-4 py-2 text-xs">敏感操作：{permission.toolName} · {permission.summary}<button type="button" className="ml-auto underline" onClick={() => void window.electronAPI.chatrooms.respondPermission({ requestId: permission.requestId, behavior: 'allow' }).then(() => setPermission(null)).catch((error) => setActionError(error instanceof Error ? error.message : '批准失败'))}>批准</button><button type="button" className="underline" onClick={() => void window.electronAPI.chatrooms.respondPermission({ requestId: permission.requestId, behavior: 'deny' }).then(() => setPermission(null)).catch((error) => setActionError(error instanceof Error ? error.message : '拒绝失败'))}>拒绝</button></div>}
