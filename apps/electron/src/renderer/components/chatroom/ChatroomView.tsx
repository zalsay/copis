...
export function ChatroomView({ roomId }: ChatroomViewProps): React.ReactElement {
  ...
  const sourceFor = (invocation: typeof invocations[number]): string | undefined => { const trigger = messages.find((message) => message.messageId === invocation.triggerMessageId); return trigger ? `${trigger.senderType === 'agent' ? 'Agent' : '用户'} ${trigger.senderId}` : undefined }
  ...
  const hasDurableReply = (invocation: typeof invocations[number]): boolean => Boolean(invocation.traceId && messages.some((message) => message.senderType === 'agent' && message.traceId === invocation.traceId))
