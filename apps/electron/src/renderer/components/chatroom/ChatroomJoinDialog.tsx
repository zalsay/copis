...
import { chatRoomRejoinRoomAtom } from '@/atoms/chatroom-atoms'

export function ChatroomJoinDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }): React.ReactElement {
  const [code, setCode] = React.useState(''); const [error, setError] = React.useState(''); const [tabs, setTabs] = useAtom(tabsAtom); const [, setActive] = useAtom(activeTabIdAtom); const rejoinRoom = useSetAtom(chatRoomRejoinRoomAtom)
  const submit = async (event: React.FormEvent): Promise<void> => { event.preventDefault(); setError(''); const shareCode = code.trim().toUpperCase(); if (!/^[A-Z0-9]{4}$/.test(shareCode)) { setError('请输入4位字母或数字分享码'); return } try { const room = await chatRoomApi.joinRoom({ shareCode }); rejoinRoom(room); const next = openChatRoomTab(tabs, room); setTabs(next.tabs); setActive(next.activeTabId); onOpenChange(false); setCode('') } catch (e) { setError(e instanceof Error ? e.message : '加入聊天室失败') } }
