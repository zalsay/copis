...
- Task 8 — Structured Output Parsing and Sanitization: complete at Copis `e634c524` with ai-education companion `fa64bcea`. Controller verified sanitizer 15/15, prompt 32/32, Shared 14/14, workspace store 23/23, Rust protocol 34/34, gateway 57/57, client 38/38, full Rust 451/451, Go handlers/services ChatRoomV2 each at `-count=5`, Electron typecheck and main/renderer builds, cargo fmt/gofmt and both repository diff checks. Final fresh Luna review: APPROVED, no Critical/Important/Minor. Accepted invariants include explicit Unicode White_Space + FEFF display-name canonicalization, atomic room-wide builder failure, strict structured output without raw `@` activation, path/secret/surrogate sanitization, complete 128 KiB WebSocket frame with 64 KiB text fields, exact Agent transient schemas, original-array mention/attachment limits before ordered deduplication, and Go/Rust content/ID control-character parity.
