...
客户端按顺序请求各候选的 `/health`，选择第一个返回 2xx 的地址，本次启动中的模型配置、Responses 请求和图片任务都复用该地址。健康探测不携带用户 Token；获取配置和全部探测共用 2 秒总超时。配置服务不可用或候选均不健康时，客户端回退到 `https://pie.meetlife.com.cn/model-request`，不会阻止 Copis 启动。
